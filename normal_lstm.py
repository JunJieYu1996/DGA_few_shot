...
class My_LSTM(nn.Module):
    ...
    def forward(self, x):
        x_embed = self.embed(x).permute(1, 0, 2) # -- only permute for conv

        # Run encoder
        x_x = self._encoder(x_embed)

        x_x = x_x[-1,:,:]

        softmax_out = self.softmax(self.dense(self.dense_dropout(x_x)))
...
class Trainer:
    ...
    def _setup_data(self):
        ...
        self.train_loader = DataLoader(test_1, sampler=train_sampler,
                                       batch_size=self.batch_size, collate_fn=collate_fn_2)
    ...
    def evaluate(self, data):
        ...
        for ix, (x, y) in enumerate(data):
            ...
            with torch.no_grad():
                ...
                probs = torch.softmax(output,dim=1).data.cpu().numpy().tolist()
                # print(type(probs))
